function[value_of_attribute] = GLCM_Contrast(Window,no_of_discritizations)
...
for i = 1:rows_of_GLCM_d10
    for j = 1:columns_of_GLCM_d10
        entropy1 = entropy1 + (((i-j).^2)*  GLCM_for_d_10(i,j) );
    end
end    
...
for i = 1:rows_of_GLCM_d01
    for j = 1:columns_of_GLCM_d01
        entropy2 = entropy2 + (((i-j).^2)*  GLCM_for_d_01(i,j) );
    end
end    
...
for i = 1:rows_of_GLCM_d11
    for j = 1:columns_of_GLCM_d11
        entropy3 = entropy3 + (((i-j).^2)*  GLCM_for_d_11(i,j) );
    end
end    
