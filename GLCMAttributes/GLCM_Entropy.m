function[value_of_attribute] = GLCM_Entropy(Window,no_of_discritizations)
...
for i = 1:rows_of_GLCM_d10
    for j = 1:columns_of_GLCM_d10
        entropy1 = entropy1 - (GLCM_for_d_10(i,j) *(log( GLCM_for_d_10(i,j))));
    end
end    
value_of_attribute = entropy1;
display(log( GLCM_for_d_10(i,j)));
%***************************************%
...
for i = 1:rows_of_GLCM_d01
    for j = 1:columns_of_GLCM_d01
        entropy2 = entropy2 - (GLCM_for_d_01(i,j) *(log( GLCM_for_d_01(i,j))));
    end
end    
if(entropy2 > value_of_attribute)
    value_of_attribute = entropy2;
end
display(log( GLCM_for_d_01(i,j)));
%***************************************%
...
for i = 1:rows_of_GLCM_d11
    for j = 1:columns_of_GLCM_d11
        entropy3 = entropy3 - (GLCM_for_d_11(i,j) *(log( GLCM_for_d_11(i,j))));
    end
end    
if(entropy3 > value_of_attribute)
    value_of_attribute = entropy3;
end
display(log( GLCM_for_d_11(i,j)));
%***************************************%